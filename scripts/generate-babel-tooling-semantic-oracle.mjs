...
const toolingHarness = path.join(
  repositoryRoot,
  'scripts/lib/compiler-tooling-semantic-harness.mjs',
)
...
function parseArguments(argv) {
  ...
  return {
    input: path.resolve(
      options.input ??
        path.join(repositoryRoot, 'scripts/fixtures/babel_0_28_tooling_semantic_inputs.json'),
    ),
    ...
    output: path.resolve(
      options.output ??
        path.join(
          repositoryRoot,
          'crates/fict-compiler/tests/babel_0_28_tooling_semantic_oracle.json',
        ),
    ),
  }
}
...
assert.equal(typeof compilerModule.analyzeFictFile, 'function', 'legacy analyzeFictFile export')
...
for (const fixture of input.fixtures) {
  ...
  assert.equal(fixture.request.moduleKind, 'module', fixture.id)
  ...
  assert.equal(typeof transformed?.metadata?.fictExplain, 'object', `${fixture.id} explain`)
  const analysis = compilerModule.analyzeFictFile(fixture.request.code, fixture.request.filename, {
    includeRegions: true,
    includeDiagnostics: true,
    verbosity: 'verbose',
    compilerOptions: fixture.request.options,
  })
  fixtures.push({
    id: fixture.id,
    babelCodeSha256: sha256(transformed.code),
    babelCode: transformed.code,
    babelWarnings: warnings,
    babelExplainSha256: sha256(JSON.stringify(transformed.metadata.fictExplain)),
    babelExplain: transformed.metadata.fictExplain,
    babelAnalysisSha256: sha256(JSON.stringify(analysis)),
    babelAnalysis: analysis,
  })
}
...
const oracle = {
  ...
  provenance: {
    ...
    toolingHarnessSha256: sha256(readFileSync(toolingHarness)),
  },
  fixtures,
}
writeFileSync(
