...
function parseArguments(argv) {
  ...
  return {
    input: path.resolve(
      options.input ??
        path.join(repositoryRoot, 'scripts/fixtures/babel_0_28_ssr_semantic_inputs.json'),
    ),
    legacyRoot: path.resolve(options['legacy-root']),
    output: path.resolve(
      options.output ??
        path.join(repositoryRoot, 'crates/fict-compiler/tests/babel_0_28_ssr_semantic_oracle.json'),
    ),
  }
}
...
const ssrPackage = JSON.parse(
  readFileSync(path.join(repositoryRoot, 'packages/ssr/package.json'), 'utf8'),
)
...
const harnessText = readFileSync(
  path.join(repositoryRoot, 'scripts/lib/compiler-ssr-semantic-harness.mjs'),
  'utf8',
)
assert.equal(input.schemaVersion, 1)
assert.equal(typeof input.publicModuleId, 'string')
...
for (const fixture of input.fixtures) {
  ...
  const transformed = transformSync(fixture.source, {
    ...
    plugins: [
      ...
      [
        compiler,
        {
          autoExtractHandlers: fixture.mode === 'resume',
          dev: false,
          emitModuleMetadata: false,
          fineGrainedDom: true,
          onWarn: warning => warnings.push(warning),
          publicModuleId: input.publicModuleId,
          resumable: fixture.mode === 'resume',
  ...
  fixtures.push({
    ...
    babelCode,
    expected: await executeSsrEsm(
      { code: babelCode, artifacts: [] },
      fixture,
      input.publicModuleId,
    ),
  })
}
