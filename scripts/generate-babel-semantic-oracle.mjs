...
function parseArguments(argv) {
  ...
  return {
    input: path.resolve(
      options.input ??
        path.join(repositoryRoot, 'scripts/fixtures/babel_0_28_semantic_inputs.json'),
    ),
    legacyRoot: path.resolve(options['legacy-root']),
    output: path.resolve(
      options.output ??
        path.join(repositoryRoot, 'crates/fict-compiler/tests/babel_0_28_semantic_oracle.json'),
    ),
  }
}
...
for (const [name, expectedVersion] of Object.entries(dependencyVersions)) {
  ...
  assert.equal(packageJson.version, expectedVersion, `${name} version`)
}
