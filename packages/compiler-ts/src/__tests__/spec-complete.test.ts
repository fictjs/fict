...
describe('R017: Ambiguity resolution', () => {
  it('function parameter shadows outer state', () => {
    ...
    expect(output).toContain('(count) => console.log(count)')
...
describe('Error cases', () => {
  ...
  it('auto-imports $effect even without explicit import', () => {
    // Compiler auto-imports runtime helpers for recognized patterns
    const output = transform(`
      $effect(() => {})
    `)
    expect(output).toContain('__fictEffect')
