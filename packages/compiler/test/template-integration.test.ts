...
describe('compiled templates DOM integration', () => {
  ...
  it('sets namespaced spread attributes on SVG and MathML descendants', async () => {
    ...
    expect(svgRoot.getAttribute('xml:lang')).toBe('en')
    expect(svgUse.getAttribute('xlink:href')).toBe('#a')
    expect(svgUse.getAttribute('xml:lang')).toBe('en')
    expect(mathRoot.getAttribute('xml:lang')).toBe('en')
    expect(mathAction.getAttribute('xlink:href')).toBe('#a')
    expect(mathAction.getAttribute('xml:lang')).toBe('en')
    ...
    expect(svgRoot.getAttribute('xml:lang')).toBe('fr')
    expect(svgUse.getAttribute('xlink:href')).toBe('#b')
    expect(svgUse.getAttribute('xml:lang')).toBe('fr')
    expect(mathRoot.getAttribute('xml:lang')).toBe('fr')
    expect(mathAction.getAttribute('xlink:href')).toBe('#b')
    expect(mathAction.getAttribute('xml:lang')).toBe('fr')
  ...
  it('keeps delegated class data evaluation inside the handler scope', async () => {
    const source = `
      import { render } from 'fict'

      export const seen: string[] = []

      function select(value: { prototype: Record<string, unknown> }) {
        seen.push(typeof value.prototype.click)
      }

      export function App() {
        return <button data-id="btn" onClick={(event) => select(class { [event.type]() {} })}>Click</button>
      }
    ...
    expect(mod.seen).toEqual(['function'])
