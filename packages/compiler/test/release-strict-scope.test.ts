...
describe('release strict guarantee scope', () => {
  ...
  it('keeps workflow-level release and CI scope aligned with root scripts', () => {
    ...
    const releasePreflight = workflowJob(releaseWorkflow, 'release-preflight')
    expect(releasePreflight).toContain('pnpm release:preflight')
    expect(releasePreflight).toContain('pnpm release:plan --offline --tag')
    expect(workflowJob(releaseWorkflow, 'release').match(/pnpm release:plan/g)).toHaveLength(2)
    ...
    expect(ciWorkflow).toContain('pnpm release:preflight')
    expect(scripts['release:preflight']).toContain('pnpm test:release-publish-plan')
