...
describe('Spec rule coverage', () => {
  ...
  it('throws when aliased reactive creators are created inside non-JSX control flow (FICT-R004)', () => {
    const cases = [
      `
        import { $state, createEffect as effect } from 'fict'
        function Demo({ ready }) {
          const count = $state(0)
          if (ready) {
      ...
      `,
      `
        import { $state, createSelector as selector } from 'fict'
        function Demo({ ready }) {
          const count = $state(0)
          if (ready) {
            selector(() => count)
  ...
  it('can downgrade aliased FICT-R004 reactive creator diagnostics to warnings', () => {
    const { warnings } = transformWithWarnings(
      `
        import { $state, createMemo as memo } from 'fict'
        function Demo({ ready }) {
  ...
  it('does not treat local reactive creator shadows as imported FICT-R004 calls', () => {
    expect(() =>
      transform(`
        import { $state } from 'fict'
        function Demo({ ready }) {
  ...
  it('throws when runtime-package reactive creators are created inside non-JSX control flow (FICT-R004)', () => {
    const cases = [
      ...
      `
        import { $state } from 'fict'
        import { createSelector } from '@fictjs/runtime/advanced'
        function Demo({ ready }) {
          const count = $state(0)
          if (ready) {
            createSelector(() => count)
      ...
      `
        ...
        function Demo({ items }) {
          const count = $state(0)
          for (const item of items) {
            Advanced.createSelector(() => count === item)
          }
          return <button>{count}</button>
        }
      `,
  ...
  it('can downgrade runtime-package FICT-R004 diagnostics to warnings', () => {
    const { warnings } = transformWithWarnings(
      `
        import { $state } from 'fict'
        import { createEffect } from '@fictjs/runtime'
    ...
    const cases = [
      `
        ...
        function Demo({ ready }) {
          const count = $state(0)
          if (ready) {
            F.createEffect(() => console.log(count))
          }
          return <button>{count}</button>
        }
      `,
  ...
  it('can downgrade namespace FICT-R004 reactive creator diagnostics to warnings', () => {
    const { warnings } = transformWithWarnings(
      `
        import { $state } from 'fict'
        import * as F from 'fict'
  ...
  it('does not treat local namespace shadows or JSX-managed namespace calls as FICT-R004', () => {
    expect(() =>
      transform(`
        import { $state } from 'fict'
        import * as F from 'fict'
        function Demo({ ready }) {
          const count = $state(0)
          const F = { createMemo: (fn: () => number) => fn() }
          if (ready) {
            F.createMemo(() => count)
          }
          return <button>{count}</button>
    ...
    expect(() =>
      transform(`
        import * as F from 'fict'
        function Demo({ ready }) {
          return <div>{ready && F.createMemo(() => 1)}</div>
        }
      `),
    ).not.toThrow(/FICT-R004/)
  })
...
describe('Rule I: Component-only state placement', () => {
  ...
  it('throws when hook-style useX is called in logical OR branches', () => {
    const input = `
      import { $state } from 'fict'
      function useCounter() {
        let count = $state(0)
  ...
  it('throws when hook-style useX is called in nullish-coalescing branches', () => {
    const input = `
      import { $state } from 'fict'
      function useCounter() {
        let count = $state(0)
