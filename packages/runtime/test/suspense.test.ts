...
import { createSignal, reactive, type FictDevtoolsHook } from '../src/advanced'
import { __fictSetSSRStreamHooks } from '../src/ssr-stream'
...
describe('Suspense', () => {
  ...
  it('does not attach fallback factory reads to the effect that suspended', async () => {
    ...
    try {
      const Child = () => {
        createEffect(() => {
          if (trigger()) throw pending.token
        })
        return 'ready'
      }
  ...
  it('does not subscribe resetKeys tracking to fallback materialization signals', async () => {
    ...
    const Child = () => {
      if (shouldSuspend()) throw pending.token
      return 'ready'
    }
    const Fallback = () => {
      fallbackRenders++
  ...
  it('keeps stream hooks pending when one token reveals another token', async () => {
    const container = document.createElement('div')
    ...
    __fictSetSSRStreamHooks({
      registerBoundary: () => 'chained-boundary',
      boundaryPending,
      boundaryResolved,
    })
  ...
  it('keeps stream hooks pending when a reset render suspends again', async () => {
    const container = document.createElement('div')
    const reset = createSignal(0)
    ...
    __fictSetSSRStreamHooks({
      registerBoundary: () => 'reset-boundary',
      boundaryPending,
      boundaryResolved,
    })

    const Child = () => {
      if (shouldSuspend) throw current.token
      return 'ready'
    }
