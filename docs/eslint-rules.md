...
| `no-state-outside-component`       | placement validation | P0    | `$state` declared at module level or outside components |
...
- `FICT-S002` is the compiler warning for state value escape (for example passing `$state` directly as a function argument), not declaration placement.
