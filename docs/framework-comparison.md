...
- **Fict vs React**: React relies on re-running components to detect changes (Diffing). The React Compiler optimizes _what_ needs to re-run (Memoization), but the model remains "top-down rendering". Fict, like Solid/Svelte, binds state changes directly to DOM updates.
