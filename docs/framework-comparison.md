...
*   **Runtime Model**: **Components run only once** (Setup phase), similar to SolidJS, but introduces "Region-based" control flow optimization to prevent graph fragmentation.
...
| **Component Execution** | **Run Once (Setup)** | Runs on every render | Run Once (Setup) | Run Once (Setup) | Run Once (Setup) |
...
*   **Fict vs React**: React relies on re-running components to detect changes (Diffing). The React Compiler optimizes *what* needs to re-run (Memoization), but the model remains "top-down rendering". Fict, like Solid/Svelte, binds state changes directly to DOM updates.
*   **Fict vs Solid**: Both build dependency graphs. Solid requires explicit read/write separation (`[count, setCount]`). Fict uses compiler magic to allow mutable syntax (`count++`) while generating the same underlying graph.
*   **Fict vs Svelte 5**: The models are very similar. Both use mutable syntax and compile to signals. The main difference is that Fict uses TSX and automatically infers derivations, whereas Svelte uses `.svelte` templates and requires `$derived`.
...
*   **Fict**:
    ...
    *   **Control Flow Regions**: Compiles `if/for` blocks into a *single* memoized "Region" that returns multiple values. This avoids the overhead of creating thousands of tiny memos for complex logic (a common issue in fine-grained reactivity).
...
*   **Svelte 5**: Compiles `.svelte` files. Handles reactivity at the statement level.
...
*   **Fict**:
    ...
    *   **Events**: The "Automatic Getter" rule ensures that if a derived value is used in an event handler, it is compiled as a getter, so it always sees the latest value. **This solves the "stale closure" trap without requiring the user to manually write getters.**
...
**Final Verdict**:
Fict is like a hybrid of **Svelte 5's syntax and SolidJS's runtime**, wrapped in a **React-like TSX experience**. By making reactivity almost entirely implicit, it pushes "Compiler-Driven Development" further than any other framework.
