...
| Guaranteed   | Official `createSelector` with a synchronous source and equality depending only on its arguments | Source is tracked in its owning root; intact named, namespace, and local import aliases share the contract                         | N/A                          |
| Guaranteed   | Proven arrays, immutable aliases, shallow copies, and dynamic element reads                      | Array replacement and index changes update consumers; fresh outer copies can be edited before replacement                          | N/A                          |
...
| Out of model | Runtime value violates a direct built-in receiver type contract                                  | `$state<T>` and direct binding/parameter annotations are caller-owned proof obligations; the compiler adds no runtime family check | Native JavaScript failure    |
...
- Direct `T[]`, tuple, and built-in receiver annotations carry the same family
  ...
  methods, constructors/species, and unknown receivers cannot prove a fresh copy.
  Editing a shallow copy's elements does not permit mutating their nested objects.
- Scalar results of intact built-in methods, such as `findIndex`, can be used in
  proven array index/count positions. This does not permit retaining an inserted
  closure or passing arbitrary objects through an unknown callback host.
