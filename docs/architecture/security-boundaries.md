...
| `SEC-SSR-01`   | High     | Fixed       | A render using legacy process DOM globals MUST be exclusive from every other SSR render, including through another loaded package copy. Ordinary renders acquire shared process reservations; compatibility mode acquires an exclusive lease, restores exact property descriptors without invoking accessors, and rolls back a partial install. Verified by `packages/ssr/test/globals.test.ts` and mixed-mode stream cases in `packages/ssr/test/streaming.test.ts`. |
...
- Ordinary renders acquire shared process reservations and may overlap each
...
- Confirm making `exposeGlobals` exclusive from all SSR renders is preferable
  to the former racy compatibility behavior.
