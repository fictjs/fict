...
| Runtime          | Cached select assignments repair externally changed duplicate or multiple selection state; selector equality classes and observer ownership remain reactive; undefined store keys retain presence tracking. | `packages/runtime/test/binding-edge-cases.test.ts`, `packages/runtime/test/signal.test.ts`, `packages/runtime/test/store.test.ts`                                                                                             |
...
The gate runs automatically in `precommit`, `release:verify`, and the CI test
job after packages are built. Run it directly from the repository root for the
shortest focused feedback:
