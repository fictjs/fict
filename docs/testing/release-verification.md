...
| Native release matrix        | Release workflow                     | All eight ADR-0002 targets execute on Node 22.18/24 on the target architecture/libc. Each of the 16 lanes binds the source revision, SPDX SBOM, tarball, and Sigstore attestation digests; a missing or skipped lane blocks facade publication.           |

The JavaScript tarball gate also MUST reject unresolved `workspace:` dependency ranges and exported files absent from the archive. Native packages use the stricter native gate because one host cannot execute all architectures and libc variants. Native artifacts MUST contain the `.node` binary, `binding-manifest.json`, `SHASUMS256.txt`, a target-filtered `fict-compiler-native.spdx.json`, the npm tarball and checksum sidecar, and offline provenance/SBOM Sigstore bundles. Certification regenerates the SBOM closure from the locked workspace and verifies the published GitHub attestations before npm mutation.
