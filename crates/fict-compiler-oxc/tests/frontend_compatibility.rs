...
#[test]
fn frozen_0_28_corpus_has_matching_rust_frontend_outcomes() {
    let fixtures: Vec<Fixture> = serde_json::from_str(include_str!("frontend_compatibility.json"))
        .expect("valid frozen frontend compatibility corpus");
    assert!(
        fixtures.len() >= 12,
    ...
    for fixture in fixtures {
        ...
        if fixture.accepted {
            ...
            let second_hir = second.hir.as_ref().expect("deterministic fixture HIR");
            verify_hir(hir).unwrap_or_else(|diagnostics| {
                panic!("{}: {:?}", fixture.name, diagnostics.as_slice())
            });
            assert_eq!(
                print_hir(hir),
                print_hir(second_hir),
                "{} must produce deterministic HIR",
                fixture.name
            );
            assert_eq!(
