...
struct AnalysisFixture {
    ...
    source: String,
    #[serde(default)]
    legacy_expected: Option<AnalysisSnapshot>,
    #[serde(default)]
...
#[test]
fn rust_analysis_matches_the_frozen_0_28_normalized_corpus() {
    let fixtures: Vec<AnalysisFixture> =
        serde_json::from_str(include_str!("analysis_compatibility.json"))
            .expect("valid frozen analysis compatibility fixtures");

    let differences: Vec<_> = fixtures
        .into_iter()
        .filter_map(|fixture| {
            assert_eq!(
                fixture.legacy_expected.is_some(),
                fixture.deviation_policy.is_some(),
                "{} must explicitly classify legacy analysis deviations",
                fixture.name
            );
            if let Some(legacy_expected) = fixture.legacy_expected.as_ref() {
                assert_ne!(legacy_expected, &fixture.expected, "{}", fixture.name);
                assert_eq!(
                    fixture.deviation_policy.as_deref(),
                    Some("narrow-component-role"),
                    "{}",
                    fixture.name
                );
            }
            let actual = analyze(&fixture);
            (actual != fixture.expected).then_some((fixture.name, actual, fixture.expected))
        })
        .collect();
    assert!(
        differences.is_empty(),
        "normalized analysis differs: {differences:#?}"
    );
}
